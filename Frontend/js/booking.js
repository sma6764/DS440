...
function loadTimeSlots(doctorId, date) {
  ...
  // Always clear container before each render cycle.
  timeSlotsContainer.innerHTML = '';
...
function handleConfirmBooking() {
  ...
  const doctorId = Number.parseInt(bookingData.doctor, 10);
  const branchId = Number.parseInt(bookingData.branch, 10);
  const appointmentDate = (bookingData.date || '').trim();
  const appointmentTime = (bookingData.time || '').trim();
  ...
  const bookingPayload = {
    doctor_id: doctorId,
    branch_id: Number.isNaN(branchId) ? 0 : branchId,
  ...
  .then(async (response) => {
    ...
    if (!response.ok) {
      return {
        ...data,
        success: false,
        message: data.message || `Booking failed (HTTP ${response.status})`
      };
    }
    return data;
  })
